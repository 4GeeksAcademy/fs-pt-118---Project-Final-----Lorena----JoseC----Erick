...
from sqlalchemy import String, Boolean, Integer, ForeignKey, DateTime,Text, Enum as SqlEnum, UniqueConstraint, func
...
class User(db.Model):
    ...
    events_created: Mapped[list["Events"]] = relationship(back_populates="creator",foreign_keys='Events.creator_id')
    # muchos-a-muchos
    events_joined: Mapped[list["Events"]] = relationship(secondary="users_events",back_populates="participants")

    # grupos creados por el usuario
    groups_created: Mapped[list["Groups"]] = relationship(back_populates="owner", foreign_keys='Groups.user_id')
    # muchos-a-muchos
    users_groups_link: Mapped[list["UsersGroups"]] = relationship(back_populates="user", cascade="all, delete-orphan")
    groups_joined: Mapped[list["Groups"]] = relationship(secondary="users_groups", back_populates="members")
    ...
    reservations: Mapped[list["Reservations"]] = relationship(back_populates="user")
    # relaciones con favoritos -------> vínculo directo a la tabla intermedia
    favorites_link: Mapped[list["Favorites"]] = relationship("Favorites",back_populates="user",cascade="all, delete-orphan",passive_deletes=True)
    # lista directa de eventos marcados como favoritos
    favorite_events: Mapped[list["Events"]] = relationship("Events",secondary="favorites",viewonly=True)
...
class Events(db.Model):
    ...
    creator: Mapped["User"] = relationship("User", back_populates="events_created",foreign_keys=[creator_id])
    # muchos-a-muchos
    participants: Mapped[list["User"]] = relationship(secondary="users_events",back_populates="events_joined")
    # muchos-a-muchos grupos(teams)
    groups: Mapped[list["Groups"]] = relationship(secondary="events_groups", back_populates="events")
    ...
    favorited_by_link: Mapped[list["Favorites"]] = relationship("Favorites",back_populates="event")
    fans: Mapped[list["User"]] = relationship("User",secondary="favorites",viewonly=True)
...
class UsersEvents(db.Model):
    ...
    __table_args__ = (
        UniqueConstraint('user_id', 'event_id', name='uq_users_events_user_event'),
        )
...
class Groups(db.Model):
    ...
    owner: Mapped["User"] = relationship("User", back_populates="groups_created",foreign_keys=[user_id])
    # muchos-a-muchos
    members: Mapped[list["User"]] = relationship(secondary="users_groups", back_populates="groups_joined")
    # realaciones para leer los extremos mas facilmente
    users_groups_link: Mapped[list["UsersGroups"]] = relationship(back_populates="group")
    # muchos-a-muchos
    events: Mapped[list["Events"]] = relationship(secondary="events_groups", back_populates="groups")
...
class UsersGroups(db.Model):
    ...
    user: Mapped["User"] = relationship("User", back_populates="users_groups_link")
    group: Mapped["Groups"] = relationship("Groups", back_populates="users_groups_link")
...
class Comments(db.Model):
    ...
    event:  Mapped["Events"] = relationship("Events", back_populates="comments")
...
class Reservations(db.Model):
    ...
    __table_args__ = (
    UniqueConstraint("user_id", "event_id", name="uq_reservations_user_event"),
)
    ...
    event: Mapped["Events"] = relationship("Events", back_populates="reservations")
...
class Favorites(db.Model):
    ...
    user: Mapped["User"] = relationship("User", back_populates="favorites_link")
    event: Mapped["Events"] = relationship("Events", back_populates="favorited_by_link")
