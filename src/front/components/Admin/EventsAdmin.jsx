...
const EventsAdmin = () => {
  ...
  const closeAlert = () => {
    setAlertModal({ show: false, message: "", type: "" })
  }
            ...
            {filteredEvents.map((event, index) => (
              <div key={event.id || index} className="card shadow-sm border-0" style={{ borderRadius: "8px" }}>
                <div className="card-body d-flex flex-wrap justify-content-between align-items-center">
                  <div className="d-flex align-items-center flex-grow-1 gap-3" style={{ minWidth: "230px", flex: "1 1 250px" }}>
                    {event.imagen && (
                      <img
                        ...
                        alt={event.name}
                        className="rounded-circle border" width="70" height="70" style={{
                          objectFit: "cover", flexShrink: 0,
                        }}
                      />
                    )}
                    <div
                      className="d-flex flex-column justify-content-center text-truncate"
                      style={{ maxWidth: "200px" }}
                    >
                      <h6
                        className="fw-semibold mb-1 text-capitalize text-truncate"
                  ...
                  <div
                    className="d-flex flex-column small text-muted"
                    style={{
                      minWidth: "220px",
                      flex: "1 1 200px",
                      textAlign: "left",
                  ...
                  <div
                    className="d-flex flex-column small text-muted text-nowrap"
                    style={{ minWidth: "100px", flex: "0 0 auto", }}
                  >
                    <span>
                      <strong>Groups:</strong> {event.groups?.length || 0}
                    </span>
                    <span>
                      <strong>Comments:</strong> {event.comments?.length || 0}
                    </span>
                  </div>
